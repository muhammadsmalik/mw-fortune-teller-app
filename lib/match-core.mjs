...
/**
 * A profile is "thin" when LinkedIn hid the structured sections from EnrichLayer
 * (no headline / occupation / industry / summary / experience) — common for
 * privacy-restricted profiles. All that survives is their posts + location, so
 * we embed those instead of letting the vector collapse onto a bare city.
 */
 ...
 *
 * Thin profiles fall back to recent post titles (the same signal
 * lib/profile-extract.mjs feeds the LLM) so they match on what they actually do,
 * not on "London, United Kingdom".
 */
export function embedTextFromProfile(p) {
  ...
  if (isThinProfile(p)) {
    const posts = (Array.isArray(p.activities) ? p.activities : [])
      .slice(0, 8)
      .map((a) => a.title)
      .filter(Boolean);
    parts.push(...posts);
