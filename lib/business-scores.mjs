...
function client() {
  if (_ai) return _ai;
  // Prefer Vertex AI when GCP creds are configured — far higher quota than the
  // AI Studio free tier (15 rpm). Falls back to an AI Studio API key otherwise.
  // Mirrors lib/match-reasons.mjs so both grounded paths authenticate the same way.
  ...
  if (!apiKey) throw new Error('Set GCP_PROJECT_ID + GOOGLE_SERVICE_ACCOUNT_JSON (Vertex) or GEMINI_API_KEY / GOOGLE_GENERATIVE_AI_API_KEY for business scoring.');
