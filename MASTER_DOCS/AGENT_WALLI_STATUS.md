...
- **Email pre-fill is RSVP-only — privacy rule (Mehul, 2026-05-30) — fixed 2026-05-31 (⚠️ needs commit + redeploy):** the concierge form must pre-populate an attendee's email **only if they explicitly shared it via the Meet-Your-Twin RSVP list**. Everyone else (non-RSVP directory entries, and **all walk-ins** — `select-name/page.js:108` hardcodes `''`) starts blank and is prompted.
  ...
  - **Fix (two parts):** (1) **data** — stripped `lib/twin_index.json` back to **40 RSVP-shared emails only** (set to the address each person actually gave on the RSVP sheet), all 383 non-RSVP blanked, verified 0 remain; (2) **build script** — `build-twin-matches.mjs` now fills the picker `email` from `rsvpEmailBySlug` **only**, never `emailBySlug`, so a future regen can't reintroduce the leak.
