...
_Last updated: 2026-06-01 · Branch: `integration/woo-e2e`_
...
- **`/concierge`** — shows the chosen matches, captures email (pre-filled **only for RSVP attendees** — see privacy note in §2 Email), sends the request. Routes to `/confirmation`.
- **`/confirmation`** — thank-you + "start over" (clears localStorage) + **opt-in CTA** to email the Agent WALLi "market read" (taps fire the grounded business-insight job; see `product/BUSINESS_INSIGHT_SCORING.md`).
...
- `POST /api/send-email` has **four** booth templates (dispatched via a `BOOTH_TEMPLATES` map): **`twinConfirmation`** (to the attendee, their matches + talking points), **`salesRepNotification`** (to the **marketing DRI**, lists requested matches w/ LinkedIn links, flags missing-email cases + which matches were auto-emailed), **`matchIntro`** (to each selected match — see below), and **`businessInsight`** (the opt-in Agent WALLi market-read scorecard + Book-a-Demo CTA, fired from `/confirmation` via `POST /api/business-insight`; has a `fallback` variant — see `product/BUSINESS_INSIGHT_SCORING.md`).
- **Match-intro emails (NEW):** on submit, each **selected** match that has an `email` gets a `matchIntro` — the attendee + role/company, the **shared list of booth networking windows** (`lib/meeting-slots.js`), and the grounded talking points; **reply-to is the requesting attendee + the concierge team** (2026-06-01 decision) so the match's "yes" reaches the requester directly while the team stays on the thread to broker/track. **Copy** (2026-06-01) frames the Moving Walls booth as an *optional* concierge-assisted meet with a "set it up for the future" fallback — no overpromised "come find them at the booth." Matches without an email are skipped and flagged in the DRI notification for a manual intro. `email` is an optional field per match in `twin_matches.json` — real values pending the CRM list (currently **test data only**, see §4).
...
- **Email test mode (NEW):** `EMAIL_TEST_MODE=true` reroutes **every** outbound email (subject prefixed `[TEST→<real recipient>]`) via a single server-side `resolveDelivery()` chokepoint — booth tests never hit a real inbox. Server-only var → keep `false` for the live event; restart to apply. **Per-tester reroute:** when test mode is on, the whole submit (attendee confirmation + DRI notification + all match-intros) is rerouted to **the email the tester typed at `/concierge`** (`testRerouteTo`), so each person testing collects their full batch in their **own** inbox. If no email is on the request it falls back to the fixed `EMAIL_TEST_RECIPIENTS` list. Ignored entirely when test mode is off.
...
- **Email pre-fill is RSVP-only — privacy rule (Mehul, 2026-05-30) — fixed + committed 2026-05-31 (⚠️ needs redeploy):** the concierge form must pre-populate an attendee's email **only if they explicitly shared it via the Meet-Your-Twin RSVP list**. Everyone else (non-RSVP directory entries, and **all walk-ins** — `select-name/page.js:108` hardcodes `''`) starts blank and is prompted.
  - **Leak found:** the picker regen at commit `dcffad3` ("regenerate picker path on full pool") populated the directory `email` field from the **enriched WOO/CRM pool** (`emailBySlug`, 881 emails), so `lib/twin_index.json` shipped **424 emails — 383 of them non-RSVP** (e.g. `stotz@stroeer.de`). The picker would have pre-filled those at the concierge form → direct violation of the privacy rule.
  - **Fix (two parts):** (1) **data** — stripped `lib/twin_index.json` back to **39 RSVP-shared emails only** (set to the address each person actually gave on the RSVP sheet), all 383 non-RSVP blanked, verified 0 remain; (2) **build script** — `build-twin-matches.mjs` now fills the picker `email` from `rsvpEmailBySlug` **only**, never `emailBySlug`, so a future regen can't reintroduce the leak.
  - **Scope note:** the enriched-pool email is **still** used for the **match-intro recipient** field in `lib/twin_matches.json` (the approved "X wants to meet you" email) — that's a separate decision, not the pre-fill rule, so it was left intact.
  - **⚠️ Action:** committed on `integration/woo-e2e` — **redeploy** before the event, since the live booth would otherwise serve the leaked `dcffad3` directory.
- **Directory dedup (2026-05-31):** removed two phantom duplicate picker records that each shared one real attendee's email. `yiannis-portokalakis-54688740` was a *different* person's profile ("Ioannis Portokalakis, Owner at Citrus Byte") auto-attached to Nikolas Portokalakis's email — removed from the enriched CSV + `pool_index.json` + the picker, keeping the real `portokalakis`. `sotos-thimio` was a `pending` stub of Efthimios "Thimio" Sotos — removed, keeping the fully-matched `efthimios-sotos-a5468722`. Root cause fixed: `build-twin-matches.mjs` now resolves an RSVP person to their pool vector by **email** when URL+name both miss (RSVP "Thimio Sotos" vs scraped "Efthimios Thimio Sotos"), so the stub can't regenerate. A full directory scan found **no other** same-name or same-email duplicates; the 5 same-surname/same-company pairs (e.g. Adam + Sally Whitford) are genuine distinct attendees and were left intact.
...
- **Final graph:** 523 source rows · **488 with matches, 35 pending** · 1,464 edges · **1,442 (98.5%) carry a Gemini reason + 3 talking points**. _(Post-dedup 2026-05-31: **521 sources · 487 with matches · 34 pending** after removing the two phantom records — see §2 "Directory dedup". 3 reveals dropped 3→2 matches where `yiannis` was a match card.)_
...
- ~~**Direct "X wants to meet you" emails to matches**~~ ✅ **DONE** — `matchIntro` emails each selected match that has an `email` (meeting slot + talking points; reply-to = **requester + team**). Still gated on **real match emails** (test data only so far — see §4). Meeting slots now ship as a shared list (`lib/meeting-slots.js`); only the final times need confirming. **Reply-to + copy + CRM routing updated 2026-06-01** — reply now reaches the requester + team (was team-only in prod); booth-or-future framing replaces the "meet at the event" overpromise; and **CRM (non-attending) matches are no longer auto-emailed** — only WOO attendees are (Option B), CRM → DRI for manual outreach. See below.
- **✅ RESOLVED 2026-06-01 — reply routing + intro copy:** the team decided WALLi should let the two parties **connect directly** while MW acts as concierge (not a team-only relay). Implemented in `app/api/send-email/route.js`: (1) `matchIntro` reply-to = **requester + concierge team** (`resolveReplyTo` → `[bodyReplyTo, ...CONCIERGE_CC_EMAILS]`), so a "yes" lands in the requester's inbox *and* the team's; (2) `matchIntro` + `twinConfirmation` copy reframed to **reply-to-connect + optional Moving Walls booth + "set it up for the future" fallback**, dropping the "come find them at the booth" / "meet at the event" overpromise.
- **✅ RESOLVED 2026-06-01 (Option B) — CRM matches are NOT auto-emailed:** of 1,464 matches, **498 (~34%) are CRM-only** (`lists:['crm']`, not attending). Decision: only **WOO attendees** (`lists` includes `'woo'`) with an email get an auto `matchIntro`; **CRM contacts are never cold-emailed** — they route to the **DRI** for human-judged manual outreach (avoids cold-outbound spam-complaint / Resend sender-reputation risk; matches the "future online meetings" intent, 30-5 demo §CRM List). Implemented:
  - **`app/concierge/page.js`** — the match-intro send loop guards on `isAttending(m)` (`Array.isArray(m.lists) && m.lists.includes('woo')`) **and** having an email; empty/missing `lists` errs toward *not* emailing. (`lists` is already carried on every chosen match — precomputed `twin_matches.json` and the walk-in `/api/match` both tag it.)
  - **`salesRepNotification`** — now renders **three** per-match states for the DRI: *emailed* (WOO + email, green), *no email — needs manual intro* (amber), and *CRM — not at event; manual outreach: &lt;email&gt;* (amber, shows the address so the DRI can reach out).
  - **`twinConfirmation`** — coordination line now branches on the attending/CRM split so it never promises a reply from someone we didn't email: all-WOO → "they'll reply directly"; all-CRM → "the team will arrange an intro for afterwards"; mixed → both, scoped. (Avoids overstating "I've let them know" for CRM picks.)
