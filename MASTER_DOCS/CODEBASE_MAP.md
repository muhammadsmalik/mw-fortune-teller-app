...
## Appendix: What's Changed Since 2025-10-07

This appendix captures changes made after the original map was generated. For full detail, run:

```bash
git log --since="2025-10-07" --oneline
```

### New: Tarot Card System

A tarot-card-based selection layer was wired into the fortune journey (commits `888d12c`, `7c4115e`, `1214db1`, `a388f5a`, `506a062`, `f3e55b9`).

- **`TacticalCardSelection.js`** — Stage 3 of the journey now renders selections as tarot cards (3 on top row, 4 on bottom). Guided card flow with reduced flicker. New tarot card artwork lives in `public/assets/` (added in `888d12c`).
- **`BlueprintDisplay.js`** — Stage 4 blueprint now includes **tarot card predictions** alongside the strategic recommendations.
- The map's original descriptions of `TacticalCardSelection` ("drag-and-drop tactical focus cards") and `BlueprintDisplay` should be read as "now tarot-themed."

### Changed: Contact-details flow (end-of-journey)

The map states `/contact-details` "auto-redirects to /archetype-discovery." **That is no longer true.**

- `270a62a` — removed the auto-redirect to `/archetype-discovery`.
- `f22610d` — added a "**Start New Journey**" button on the success screen instead.
- `b0f4d68` — added a **product links** section to the success screen after email submission (see `MASTER_DOCS/product-links.md` for the link set: Studio, Planner, Market, Activate, Influence, Measure, Science).

`/archetype-discovery` still exists as a route but is no longer part of the default user flow.

### Changed: `/api/submit-lead`

`ff33f94` significantly expanded what gets written to Sheets:

- Now captures LinkedIn URL **plus** the user's journey selections (high-level + tactical).
- Append anchored to cell A1 (was previously appending without a fixed anchor).
- Added structured client-side and server-side logging for debugging submission failures.

### New: QR upload in `/collect-info`

`8ad5574` — users can now upload a QR-code image as a fallback when the live scanner can't read it.

### Copy / styling updates (non-structural)

- `b20ecb9` — landing page sub-headline + layout refresh.
- `8238fab` — switched to **British English** spellings; updated Media Agency Q2 copy.
- `ea97ee2` — broad copy + styling pass across the fortune journey.

### Versions / dependencies

- Next.js bumped **`15.3.2` → `15.3.8`** as part of the React Server Components CVE fix (`9234f26`, merged via `9dd24d6`). The dependency table in the main map still lists `15.3.2`.
- React hooks ESLint warnings cleaned up (`f757ae8`).

### New planning documents in `MASTER_DOCS/`

Files in `MASTER_DOCS/` not referenced by the original map:

- **`SATELLITE_EVENT_V1_PLAN.md`** — current/next initiative: WOO Forum satellite event. The Fortune Teller is being repurposed as a **networking + lead-gen tool** ("twinning" to pre-match attendees). See also `MEETINGS/revival-mehul-12-may.md` for the kickoff context.
- **`product-links.md`** — Moving Walls product link reference used by the new contact-details success screen.
- **`PHASE_3_DETAILED_PLAN.md`**, **`TAROT_CARD_DESIGN.md`**, **`TAROT_CARD_MAPPING.md`**, **`PERSONA_PLATFORM_MAPPING.md`**, **`OPTION_C_IMPLEMENTATION_PLAN.md`** — design notes that informed the tarot rollout and persona mapping work.

### POC scripts (`scripts/`)

The `scripts/` directory (not in the original map) contains POC code for the next phase:

- `scripts/poc-llm-insights.mjs` — Gemini + Google Search grounding to enrich RSVP rows into attendee dossiers; auto-scores Discoverability + Measurement signals; generates "why you two" twinning rationale.
- `scripts/poc-llm-only-scoring.mjs`, `scripts/poc-scoring.mjs` — scoring experiments.
- `scripts/output/` — gitignored, holds POC outputs.

These are research-only and not wired into the app yet.

### Still accurate

The architecture overview, env-var list, API route inventory, both user-flow diagrams (LinkedIn + Manual), data-flow diagram, and deprecated-code section are still correct. The main differences are the tarot layer on top of Stage 3/4, the new end-of-journey behavior, and the upcoming pivot toward networking/twinning described in `SATELLITE_EVENT_V1_PLAN.md`.

**Appendix updated:** 2026-05-18
